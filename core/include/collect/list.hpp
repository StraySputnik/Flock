...
namespace flock {
    ...
    public:
        ...
        T pop_last() {
            FLK_ASSERT(head_, "Pop on empty list");

            element *elem = head_;
            for (usize i = 0; i + 2 < len_; i++) {
                elem = elem->next;
            }
            ...
            if (len_ == 1) {
                T val = head_->value;
                head_->value.~T();
                allocator_->deallocate((byte *)head_, sizeof(element));
                head_ = nullptr;
            ...
            elem->next->value.~T();
            allocator_->deallocate((byte *)elem->next, sizeof(element));
            elem->next = nullptr;
        ...
        T remove(usize index) {
            ...
            T value = elem->value;
            elem->value.~T();
            allocator_->deallocate((byte *)elem, sizeof(element));
    ...
    public:
        ...
        template <typename... Args>
            requires (std::same_as<Args, T> && ...)
        static forward_list with_elements(Args... args) {
            forward_list out;
            out.allocator_ = memory::get_allocator();
            out.head_      = nullptr;
            out.len_       = 0;
        ...
        void append(const forward_list &other) {
            const element *elem = other.head_;
            while (elem) {
                push_last(elem->value);
                elem = elem->next;
